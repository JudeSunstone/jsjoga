...
https://www.chaijs.com/
 ...
 мока - это оснва, чтобы тесты работали, а чай - дополнение к ней 
...
в ътмл в конце пишем через link цсс и через тег script - js
то же самое для ЧАЙ
...
это все можно оптимихировать, но для наглядности так

наш скрипт загружаетя только после загрузки dom
чтобы тест сработал правильно, то это надо изменить 
самый простой вариант - удаление обработчика событий

тестируем в файле тест, что мы подключили, там ничего нет, 

describe("таймер"б, fucntion(){
    it("возвращает ли объект эта функция". function(){
        assert.typeOf(getTimerRemainning(),  "object") - проверянм ту функцию, что хротим здесь, вторым аргументом передаем обхект 
        сохарняем и смотрим на страницу
    })
}) 
